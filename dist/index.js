...
function fieldToColumnType(field) {
  if (isReal(field)) return import_driver_adapter_utils.ColumnTypeEnum.Numeric;
  if (isFloat(field)) return import_driver_adapter_utils.ColumnTypeEnum.Float;
  if (isDouble(field)) return import_driver_adapter_utils.ColumnTypeEnum.Double;
  if (isInt32(field)) return import_driver_adapter_utils.ColumnTypeEnum.Int32;
  if (isInt64(field)) return import_driver_adapter_utils.ColumnTypeEnum.Int64;
  if (isDateTime(field)) return import_driver_adapter_utils.ColumnTypeEnum.DateTime;
  if (isTime(field)) return import_driver_adapter_utils.ColumnTypeEnum.Time;
  if (isDate(field)) return import_driver_adapter_utils.ColumnTypeEnum.Date;
  if (isText(field)) return import_driver_adapter_utils.ColumnTypeEnum.Text;
  if (isBytes(field)) return import_driver_adapter_utils.ColumnTypeEnum.Bytes;
  if (isBool(field)) return import_driver_adapter_utils.ColumnTypeEnum.Boolean;
  if (isJson(field)) return import_driver_adapter_utils.ColumnTypeEnum.Json;
  if (isEnum(field)) return import_driver_adapter_utils.ColumnTypeEnum.Enum;
  if (isNull(field)) return import_driver_adapter_utils.ColumnTypeEnum.Int32;
...
// src/mysql.ts
var debug = (0, import_driver_adapter_utils2.Debug)("prisma:driver-adapter:mysql");
var MySqlQueryable = class {
  ...
  async queryRaw(query) {
    ...
    try {
      ...
      if (error instanceof UnsupportedNativeDataType) {
        return (0, import_driver_adapter_utils2.err)({ kind: "UnsupportedNativeDataType", type: error.type });
      }
      throw error;
    }
  ...
  async performIO(options) {
    try {
      ...
      return (0, import_driver_adapter_utils2.ok)({ data, fields });
    } catch (e) {
      const error = e;
...
var PrismaMySql = class extends MySqlQueryable {
  ...
  async startTransaction() {
    ...
    return (0, import_driver_adapter_utils2.ok)(new MySqlTransaction(connection, options));
  }
};
